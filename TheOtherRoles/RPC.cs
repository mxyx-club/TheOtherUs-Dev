...
public static class RPCProcedure
{
    ...
    public static void hostKill(byte targetId)
    {
        var target = playerById(targetId);
        target.Exiled();
        OverrideDeathReasonAndKiller(target, CustomDeathReason.CmdKill, GetHostPlayer);
...
internal class RPCHandlerPatch
{
    ...
    private static void Postfix([HarmonyArgument(0)] byte callId, [HarmonyArgument(1)] MessageReader reader)
    {
        ...
        if (DebugMode) Info($"接收 PlayerControl 原版Rpc RpcId{callId} Message Size {reader.Length}", "RPC");
    }
    ...
    private static bool Prefix([HarmonyArgument(0)] byte callId, [HarmonyArgument(1)] MessageReader reader)
    {
        ...
        if (DebugMode && callId != 95) Info($"接收 PlayerControl CustomRpc RpcId{callId} Rpc {RpcNames?[(CustomRPC)callId] ?? nameof(packetId)} Message Size {reader.Length}", "RPC");
        switch (packetId)
        {
            // Main Controls
