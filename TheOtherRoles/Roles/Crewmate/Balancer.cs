...
public class Balancer : RoleBase
{
    ...
    public Balancer(PlayerControl p) : base(p, roleinfo) { }

    private static void AddOptions()
    {
        balancerCount = CustomOption.Create(roleinfo.ConfigId + 3, CustomOptionType.Crewmate, "balancerCount", 1, 1, 3, 1, roleinfo.RoleOption);
        balancerVoteTime = CustomOption.Create(roleinfo.ConfigId + 4, CustomOptionType.Crewmate, "balancerVoteTime", 40, 15, 90, 5, roleinfo.RoleOption);
    }
    ...
    //public Balancer(PlayerControl Player) : base (Player, roleinfo) { }
    ...
    private static void BalancerOnClick(int Index, MeetingHud __instance)
    {
        ...
        if (currentAbilityUser.IsDead() || IsAbilityUsed <= 0) return;
    ...
    public override void OnExileWrapUp(GameData.PlayerInfo exiled)
    {
        if (exiled != null)
        {
